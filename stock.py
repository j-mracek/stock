    ...
    def parse(self,run,arg):
        try:
            ...
            try:
                ...
                analyser.message +='The function %s%s didnt work properly, may be unable to open url. \n' % (run.__name__,arg) 
                analyser.counter +=1
            ...
            for parameter in self.parameters:
                if parameter[2] is "<":
                    if parameter[1]< data[self.date]["currency"][parameter[0]][0]:
                        analyser.message +='The exchange rate for 1 %s is %s CZK and this is higher than set allert value (%s CZK).\n' % (parameter[0],data[self.date]["currency"][parameter[0]][0],parameter[1])
                        analyser.counter+=1
                elif parameter[2] is ">":   
                    if parameter[1]> data[self.date]["currency"][parameter[0]][0]:
                        analyser.message +='The exchange rate for 1 %s is %s CZK and this is lower than set allert value (%s CZK).\n' % (parameter[0],data[self.date]["currency"][parameter[0]][0],parameter[1])
                        analyser.counter+=1
                else:
